...
async def process_urls_concurrently(url):
    ...
    async with async_playwright() as p:
        ...
        locator = page.locator('#page-header-desktop-search-location')
        await locator.wait_for(state='visible', timeout=10000)
        # Fill “Seattle”
        await locator.fill("minneapolis")

        # Wait for suggestion list item to appear
        await page.wait_for_selector('#react-autowhatever-1--item-0', timeout=10000)

        # Click first suggestion
        await page.click('#react-autowhatever-1--item-0')

        await page.wait_for_load_state("networkidle")
        await page.wait_for_timeout(5000)  # wait for results to render
